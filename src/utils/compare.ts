 ...
 * 计算单个套餐的有效单价
 */
export function calcEffectivePrice(plan: TokenPlan, inputRatio = 0.7, cacheHitRate = 0): number {
  // 资源包类型：按包价/包量算
 ...
 */
export function calcValueScore(effectivePrice: number, capabilityScore: number): number {
 ...
 * 生成推荐理由
 */
function generateReason(plan: TokenPlan, result: CompareResult): string {
...
export function comparePlans(
  ...
  const results: CompareResult[] = filtered.map((plan) => {
 ...
 * 格式化对比结果为可读文本
 */
export function formatCompareResults(results: CompareResult[], topN = 10): string {
  if (results.length === 0) {
    return '没有找到符合条件的套餐。';
  }
 ...
 * 按使用场景给出推荐
 */
export function recommendByUseCase(
  plans: TokenPlan[],
  useCase: UseCase,
  budget: number,
): { best: CompareResult; alternatives: CompareResult[] } {
