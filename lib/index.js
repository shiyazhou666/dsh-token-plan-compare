...
export function apply(ctx, config) {
    ...
    ctx.tools.register(defineTool({
        ...
        description: '对比各家大模型 API 的 token 套餐性价比。当用户询问"哪个大模型便宜"、"token 价格对比"、"API 性价比"、"买哪个套餐划算"、"月预算X元用什么模型"时使用。返回按性价比排序的套餐列表。',
        parameters: {
            monthly_budget: {
                type: 'number',
    ...
    ctx.tools.register(defineTool({
        ...
        description: '根据使用场景和预算直接推荐最划算的大模型套餐。当用户说"写代码用什么模型"、"长文本选哪个"、"预算X元推荐API"时使用。',
        parameters: {
            use_case: {
                type: 'string',
                description: '使用场景：code/writing/chat/long-context/reasoning',
    ...
    ctx.tools.register(defineTool({
        ...
        description: '列出数据库中所有大模型 token 套餐的基本信息。当用户想了解"有哪些大模型API"、"支持哪些厂商"时使用。',
        parameters: {
            region: {
                type: 'string',
